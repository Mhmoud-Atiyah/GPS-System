...
web inline void *mg_calloc(size_t cnt, size_t size) {
...
web inline int mg_mkdir(const char *path, mode_t mode) {
  (void) path, (void) mode;
  return -1;
}
...
web inline int mg_getsockname(MG_SOCKET_TYPE fd, void *buf, socklen_t *len) {
  (void) fd, (void) buf, (void) len;
  return -1;
}

web inline int mg_getpeername(MG_SOCKET_TYPE fd, void *buf, socklen_t *len) {
  (void) fd, (void) buf, (void) len;
  return 0;
}
#endif
...
#define MG_TCPIP_DRIVER_INIT(mgr)                                \
  do {                                                           \
    web struct mg_tcpip_driver_imxrt_data driver_data_;       \
    web struct mg_tcpip_if mif_;                              \
...
#define MG_TCPIP_DRIVER_INIT(mgr)                                 \
  do {                                                            \
    web struct mg_tcpip_driver_pico_w_data driver_data_;       \
    web struct mg_tcpip_if mif_;                               \
...
#define MG_TCPIP_DRIVER_INIT(mgr)                                 \
  do {                                                            \
    web struct mg_tcpip_driver_stm32f_data driver_data_;       \
    web struct mg_tcpip_if mif_;                               \
    driver_data_.mdc_cr = MG_DRIVER_MDC_CR;                       \
    driver_data_.phy_addr = MG_TCPIP_PHY_ADDR;                    \
    mif_.ip = MG_TCPIP_IP;                                        \
...
#define MG_TCPIP_DRIVER_INIT(mgr)                                 \
  do {                                                            \
    web struct mg_tcpip_driver_stm32h_data driver_data_;       \
    web struct mg_tcpip_if mif_;                               \
    driver_data_.mdc_cr = MG_DRIVER_MDC_CR;                       \
    driver_data_.phy_addr = MG_TCPIP_PHY_ADDR;                    \
...
#define MG_TCPIP_DRIVER_INIT(mgr)                               \
  do {                                                          \
    web struct mg_tcpip_driver_tm4c_data driver_data_;       \
    web struct mg_tcpip_if mif_;                             \
    driver_data_.mdc_cr = MG_DRIVER_MDC_CR;                     \
    mif_.ip = MG_TCPIP_IP;                                      \
    mif_.mask = MG_TCPIP_MASK;                                  \
...
#define MG_TCPIP_DRIVER_INIT(mgr)                               \
  do {                                                          \
    web struct mg_tcpip_driver_tms570_data driver_data_;     \
    web struct mg_tcpip_if mif_;                             \
    driver_data_.mdc_cr = MG_DRIVER_MDC_CR;                     \
    driver_data_.phy_addr = MG_TCPIP_PHY_ADDR;                  \
    mif_.ip = MG_TCPIP_IP;                                      \
...
#define MG_TCPIP_DRIVER_INIT(mgr)                                 \
  do {                                                            \
    web struct mg_tcpip_driver_xmc7_data driver_data_;       \
    web struct mg_tcpip_if mif_;                               \
    driver_data_.mdc_cr = MG_DRIVER_MDC_CR;                       \
    driver_data_.phy_addr = MG_TCPIP_PHY_ADDR;                    \
    mif_.ip = MG_TCPIP_IP;                                        \
...
#define MG_TCPIP_DRIVER_INIT(mgr)                                 \
  do {                                                            \
    web struct mg_tcpip_driver_xmc_data driver_data_;       \
    web struct mg_tcpip_if mif_;                               \
    driver_data_.mdc_cr = MG_DRIVER_MDC_CR;                       \
    driver_data_.phy_addr = MG_TCPIP_PHY_ADDR;                    \
    mif_.ip = MG_TCPIP_IP;                                        \
